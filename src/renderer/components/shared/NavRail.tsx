  ...
  const renderItem = (item: NavItem) => {
    ...
    return (
      <div key={item.id} className="relative">
        ...
        >
          <Icon />
          <span className={`text-[11px] font-medium leading-none ${isActive ? '' : 'opacity-70'}`}>{item.shortLabel}</span>
        </button>
      </div>
    )
  }
        ...
        <div className="flex flex-col items-center gap-1 px-2 pb-2 no-drag">
          ...
          {sidebarCollapsed && (
            <button onClick={onToggleSidebar} title="展开侧栏" aria-label="展开侧栏"
              ...
              <span className="text-[11px] font-medium leading-none opacity-70">侧栏</span>
            </button>
          )}
          {editing && (
            <button onClick={onExitEdit} title="退出编辑" aria-label="退出编辑"
              ...
              <span className="text-[11px] font-medium leading-none text-red-500">退出</span>
            </button>
          )}
        </div>
      )}
