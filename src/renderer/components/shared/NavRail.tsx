...
export default function NavRail({ active, onChange, editing, sidebarCollapsed, onExitEdit, onToggleSidebar, onLogoClick }: Props) {
  const renderItem = (item: NavItem) => {
    ...
    return (
      <div key={item.id} className="relative">
        ...
        >
          <Icon />
          <span className={`text-[12px] font-medium leading-none ${isActive ? '' : 'opacity-70'}`}>{item.shortLabel}</span>
        </button>
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      <div style={{ height: isMac ? 39 : 8 }} className="shrink-0" />

      {/* Logo + app name */}
      <div className="flex flex-col items-center px-2 w-full no-drag">
        <button onClick={onLogoClick} title="关于 DeclarAI" aria-label="关于 DeclarAI"
          className="flex flex-col items-center gap-0.5 pb-3 cursor-pointer border-none bg-transparent hover:opacity-80 transition-opacity">
          <Logo size={30} />
          <span className="text-[12px] font-semibold text-ink leading-none">DeclarAI</span>
        </button>
      </div>
      ...
      {/* Main nav items */}
      <div className="flex flex-col items-center gap-1 px-2 no-drag">
        {navItems.map(renderItem)}
      </div>
      <div style={{ flex: '1 1 55%' }} />

      {/* Sidebar toggle + edit exit */}
      {(sidebarCollapsed || editing) && (
        <div className="flex flex-col items-center gap-1 px-2 pb-2 no-drag">
          <div className="w-10 h-px bg-gray-200 dark:bg-gray-700 mb-1" />
          {sidebarCollapsed && (
            <button onClick={onToggleSidebar} title="展开侧栏" aria-label="展开侧栏"
              className="flex flex-col items-center gap-0.5 py-2 px-3 cursor-pointer border-none bg-transparent hover:bg-slate-100 dark:hover:bg-gray-800 rounded-lg transition-colors"
              style={{ color: 'var(--muted)' }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                <rect x="3" y="3" width="18" height="18" rx="2" /><path d="M9 3v18" />
              </svg>
              <span className="text-[12px] font-medium leading-none opacity-70">侧栏</span>
            </button>
          )}
          {editing && (
            <button onClick={onExitEdit} title="退出编辑" aria-label="退出编辑"
              ...
              <span className="text-[12px] font-medium leading-none text-red-500">退出</span>
            </button>
          )}
        </div>
      )}
      ...
      {/* Bottom items */}
      <div className="flex flex-col items-center gap-1 px-2 pb-4 no-drag">
