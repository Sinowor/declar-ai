...
function formatLookupText(t: TariffData): string {
  return [`HS 编码: ${t.hs_code}  ${t.description}`, `最惠国税率: ${t.mfn_rate != null ? t.mfn_rate + '%' : '—'}`, `普通税率: ${t.general_rate != null ? t.general_rate + '%' : '—'}`, `增值税率: ${t.vat_rate}%`, `消费税率: ${t.has_consumption_tax ? '适用' : '不适用'}`, `监管条件: ${t.supervision || '无'}`, `法定单位: ${t.unit}`].join('\n')
}
